...
/* -------------------------------------------------------------------------
   ...
   - "Recent Community Reports" are static sample data for demonstration.
...
// Static sample reports (no backend / no database — for demo purposes only)
const SAMPLE_FEEDBACK_REPORTS = [
  {
  ...
  {
    gnDivision: 'Wellawatte North',
    service: 'Hospitals',
    issue: 'Hospital is difficult to access because of heavy traffic.',
    submitted: '19 July 2026'
  },
  {
    gnDivision: 'Kollupitiya',
    service: 'Schools',
    issue: 'Nearest school requires crossing a busy main road with no signal.',
    submitted: '18 July 2026'
  },
  {
    gnDivision: 'Mount Lavinia South',
    service: 'Banks',
    issue: 'Only one bank branch serves this entire division.',
    submitted: '17 July 2026'
  },
  {
    gnDivision: 'Ratmalana',
    service: 'Parks',
    issue: 'No public park or green space within a 15-minute walk.',
    submitted: '16 July 2026'
  }
];

// Reports submitted by the visitor during this browser session (not saved
// anywhere — resets on page reload). Kept separate from the static sample
// data above so the two lists can be shown in their own labeled spaces.
const SUBMITTED_FEEDBACK_REPORTS = [];
...
function renderFeedbackReports() {
  const list = document.getElementById('feedbackReportList');
  if (list) {
    list.innerHTML = SAMPLE_FEEDBACK_REPORTS.map(r => buildFeedbackCardHtml(r, false)).join('');
  }
  renderSubmittedFeedbackReports();
}

// Renders the dedicated "Your Submitted Reports" space — starts empty with
// a placeholder note, then fills in as the visitor submits the form.
...
if (submitFeedbackBtn) {
  submitFeedbackBtn.addEventListener('click', () => {
    ...
    // Also open the real Google Form so the report can be officially recorded
