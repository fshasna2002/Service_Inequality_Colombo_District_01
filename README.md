...
- **Community Feedback panel** (new) — an inline form (GN Division,
  ...
  that adds the new report to a dedicated **"Your Submitted Reports"**
  space (kept separate from the static sample reports) and opens a linked
  Google Form in a new tab for official submission
...
**Two separate report spaces:**

- **Recent Community Reports** — fixed sample data, always the same on
  every page load. Defined in `SAMPLE_FEEDBACK_REPORTS` in `script.js`.
- **Your Submitted Reports** — starts empty with a placeholder note
  ("Reports you submit above will appear here.") and fills in, newest
  first with a small "New" badge, as the visitor submits the form.
  This list is session-only (kept in a plain JS array,
  `SUBMITTED_FEEDBACK_REPORTS`) — it resets on page reload and is never
  saved anywhere.
...
2. Clicking **Submit Feedback**:
   ...
   - Adds a new card to the top of **Your Submitted Reports** (not mixed
     into the static sample list above it).
...
**Seed / sample reports** shown under "Recent Community Reports" are
defined in the `SAMPLE_FEEDBACK_REPORTS` array in the same section. Edit,
add, or remove entries there — each report follows this shape:
...
switch to — the rendering functions already re-render from whatever
array they're given, so no other code needs to change.
